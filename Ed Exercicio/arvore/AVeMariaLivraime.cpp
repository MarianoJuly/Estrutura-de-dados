...
int noh::fatorBalanceamento() {
    int altEsq = this-> esq -> altura;
    int altDir = this-> dir -> altura;
...
noh* avl::arrumaBalanceamento(noh* umNoh) {
    ...
    if((fatorBal>=-1 and fatorBal<=1)){
        ...
        }else if((fatorBal<-1) and (fatorBalanceamento(umNoh.dir))){
...
noh* avl::removeAux(noh* umNoh, tipoChave chave) {
    ...
    }else if(chave >umNoh->elemento->dado){
        umNoh.dir = removeAux(umNoh.direito, chave);
    }else{
        if(umNoh.esq == NULL){
            novaRaiz = umNoh.dir;
